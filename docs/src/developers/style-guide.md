...
!!! info
    The style guide is always a work in progress, and not all LinearElasticityRecipes code
    follows the rules. When modifying LinearElasticityRecipes, please fix the style violations
...
LinearElasticityRecipes uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/LinearElasticityRecipes.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the LinearElasticityRecipes directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to LinearElasticityRecipes have
