...
[LinearElasticityRecipes](https://github.com/MineralsCloud/LinearElasticityRecipes.jl).
...
## Install LinearElasticityRecipes
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("LinearElasticityRecipes")
   ```

3. Run

   ```julia-repl
   julia> using LinearElasticityRecipes
   ```
...
version of LinearElasticityRecipes, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/LinearElasticityRecipes.jl"
```

in the second step above.

## Update LinearElasticityRecipes
...
our [GitHub repository](https://github.com/MineralsCloud/LinearElasticityRecipes.jl)
for new releases.
Once we release a new version, you can update LinearElasticityRecipes by typing

```@repl
using Pkg
Pkg.update("LinearElasticityRecipes")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall LinearElasticityRecipes
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("LinearElasticityRecipes")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall LinearElasticityRecipes.
